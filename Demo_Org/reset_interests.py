...
from app import people, plates
from app.config import PERSISTENT_PERSONS, PERSISTENT_PLATES
...
def check_people(safe, to_delete, persons):
    ...
    def handle_trust_level(level):
        if level == "1":
            safe_names = [
                people.print_person_name(person_id, persons)
        ...
        if trust_level in ["1", "2", "3"]:
            if handle_trust_level(trust_level):
                if trust_level in ["1", "2"]:
                    people.purge_people(to_delete, persons)
                return
        else:
...
def check_plates(safe, to_delete, license_plates):
    ...
    def handle_trust_level(level):
        if level == "1":
            safe_names = [
                plates.print_plate_name(plate_id, license) for plate_id in safe
            ]
        ...
        if level == "2":
            delete_names = [
                plates.print_plate_name(plate_id, license)
                for plate_id in to_delete
            ]
            print_comparison(
    ...
    while True:
        ...
        if trust_level in ["1", "2", "3"]:
            if handle_trust_level(trust_level):
                if trust_level in ["1", "2"]:
                    plates.purge_plates(to_delete, license_plates)
                return
        else:
            print("Invalid input. Please enter '1', '2', or '3'.")
